...
//C:\\Users\\Βασίλης\\workspace\\LibraryTest_v0.1\\
	static final String DATABASE_URL = "jdbc:sqlite:testdb.sqlite"; //το ονομα της βασης
	...
	public void establishConnection(){ //κάνει connect στη βάση
		...
		try{
			...
			statement = connection.createStatement(); //για να εκτελέσει το connection(?)
			//JOptionPane.showMessageDialog(null,"connection successful");
		}
		catch(SQLException e){
			e.printStackTrace(); //για την κονσόλα
			JOptionPane.showMessageDialog(null,"Connection couldn't be established.");
		}catch(Exception e){
			e.printStackTrace();
			JOptionPane.showMessageDialog(null,"Connection couldn't be established");
		}
	...
	public void insertingIntoDB(String table, int isbn, String name){
		...
		try{
			...
			String sql = "INSERT INTO "+table+" VALUES ("+ isbn +" ,'" + name + "' )";
	...
	/*public int getResultsForADay(String table,String month,String day){
		int counter = 0;
		ResultSet resultSet = null;

		try{
			String sql = "SELECT * FROM "+table+" WHERE MONTH='"+month+"' AND DAY='"+day+"'";
	...
	public void printData(){  //εμφανιση στην κονσολα
		...
		try{
			connection = DriverManager.getConnection(DATABASE_URL); //ισως να μην χρειάζεται.. check it!!!
			statement = connection.createStatement();
			String sql = "SELECT * FROM book"; //μονο για book
			resultSet = statement.executeQuery(sql);

			ResultSetMetaData metaData = resultSet.getMetaData(); //einai ta columns tou table
			int numberOfColumns = metaData.getColumnCount();

			System.out.println( "First Table's DB concent results:\n" );

			for ( int i = 1; i <= numberOfColumns; i++ ) //i=1 (!)
